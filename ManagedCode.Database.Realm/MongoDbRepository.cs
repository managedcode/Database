...
public class RealmRepository<TItem> : BaseRepository<ObjectId, TItem>, IRealmRepository<TItem>
    where TItem : class, IItem<ObjectId>
{
    private readonly IMongoCollection<TItem> _collection;

    public RealmRepository([NotNull] RealmDbRepositoryOptions options)
    {
        var client = Realm.GetInstance();// new MongoClient(options.ConnectionString);
